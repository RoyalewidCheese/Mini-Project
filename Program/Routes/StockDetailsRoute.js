  ...
  db.getConnection((err, connection) => {
    ...
    connection.query(deleteStockQuery, [itemId], (err, result) => {
      ...
      connection.query(deleteItemsQuery, [itemId], (err, result) => {
        ...
        // Insert an audit log record for the deletion
        const userId = req.session.user.user_id; // Assuming you have a user ID available in req.user.id
        const action = `Deleted item with ID ${itemId}`;
        insertAuditLog(userId, action, (error, auditLogResult) => {
          if (error) {
