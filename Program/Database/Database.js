...
// Function to delete a borrower from the Borrow_List table
function deleteBorrower(borrowerId, callback) {
  const query = "DELETE FROM Borrow_List WHERE borrower_id = ?";
  pool.query(query, [borrowerId], (error, results) => {
    if (error) {
...
// Function to delete a borrower from the Item_Borrower table
function deleteItemBorrower(borrowerId, callback) {
  const query = "DELETE FROM Item_Borrower WHERE borrower_id = ?";
  pool.query(query, [borrowerId], (error, results) => {
    if (error) {
      console.error("Error deleting item borrower from Item_Borrower table:", error);
      callback(error);
    } else {
      callback(null, results);
    }
